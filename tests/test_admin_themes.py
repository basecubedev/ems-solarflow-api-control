...
DENSITY_STORAGE_KEY = "ems-admin-density"
...
# rhythm of the page.
UNSCALED = {
    # `code` is sized in rem against the type it holds, so it follows the font
    # rather than the page; scaling it too would move it twice.
    ("code", "padding"),
}
...
def densities():
    ...
    assert found, "admin.css declares no :root[data-density=…] blocks"
    return found


def densities_offered():
    block = re.search(r"const DENSITIES = \[(.*?)\];", JS, re.S)
    assert block, "admin.js declares no DENSITIES table"
    return dict(re.findall(r'id:\s*"([a-z0-9-]+)",\s*label:\s*"([^"]+)"', block.group(1)))
...
def test_both_halves_agree_on_where_the_density_is_stored():
    assert DENSITY_STORAGE_KEY in JS, f"admin.js does not write {DENSITY_STORAGE_KEY!r}"
