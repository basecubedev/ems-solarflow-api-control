...
def test_negative_target_publishes_a_charge_for_a_measured_model_e2e():
    ...
    assert dev.write_output_limit(-500) is True
    published = [
        r for r in broker.publish_calls if r.topic.endswith("properties/write")
    ]
    assert len(published) == 1
    properties = json.loads(published[-1].payload)["properties"]
    # A charge travels as a positive inputLimit; outputLimit is parked at zero,
    # so the wire never carries a negative one.
    assert properties == {
        "smartMode": 1,
        "acMode": 1,
        "outputLimit": 0,
