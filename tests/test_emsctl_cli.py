...
def test_emsctl_completion_bash_contains_commands_and_configured_device(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    assert "status system device ha ha-control winter dashboard influx diagnose interactive menu examples completion help" in result.stdout
...
def test_emsctl_completion_zsh_contains_commands_and_configured_device(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    assert "commands=(status system device ha ha-control winter dashboard influx diagnose interactive menu examples completion help)" in result.stdout
