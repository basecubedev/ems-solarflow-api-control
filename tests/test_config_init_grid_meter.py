...
def test_supported_grid_meter_types_include_zendure_http_types():
    # The generic local-HTTP type is the offered choice; the 3CT type stays
...
def test_supported_grid_meter_types_include_zendure_d0_http():
    # A config that names the D0 local-API type must round-trip through the CLI
    # assistant instead of being reset to Shelly.
    assert (
...
def test_ask_grid_meter_zendure_d0_http_keeps_only_ip():
    existing = {
    ...
    assert result["type"] == "zendure_smartmeter_d0_http"
