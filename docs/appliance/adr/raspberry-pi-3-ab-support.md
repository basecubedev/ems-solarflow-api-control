# ADR: no Raspberry Pi 3 profile for the A/B appliance image

- Status: accepted
- Date: 2026-08-14
- Scope: the A/B appliance image and its build profiles. Not the Docker
  deployment, not the single-slot `.deb`, not EMS control behaviour.
...
**Do not add a `rpi3` build profile.** `rpi4` and `rpi5` remain the two
profiles, and a Raspberry Pi 3 remains a board the appliance does not recognise
— which already blocks an OS update rather than guessing at one.
...
- The build profiles stay `rpi4` and `rpi5`. The release matrix does not gain a
  third image, and no gate is weakened or skipped for a Pi 3.
- A Raspberry Pi 3 reports no board class, so
  `board_is_installable()` is false and an OS update is refused with
  `hardware_not_supported`. That path already existed; this ADR is why it is
  correct rather than a gap.
- **Nothing here says a Pi 3 cannot run EMS.** The A/B *image* is what it cannot
  boot. The Docker deployment and the single-slot `.deb` have no GPT, tryboot or
  EEPROM dependency at all, and every runtime container this project deploys
  publishes a `linux/arm64` manifest. That path is untested on a Pi 3 and is not
  claimed as supported — see
  [../../user/hardware-requirements.md](../../user/hardware-requirements.md) —
  but it is a different question with a different answer.
- If upstream widens `image-rota`'s device-class rule, the first test in
  ...
  trigger to revisit this decision, and it should be revisited then rather than
  assumed still true.
