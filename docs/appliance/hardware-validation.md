...
package, and it has its own rows in the results table below. All three halves
have now happened on a Pi 3B+: an appliance has fetched and installed one over
HTTPS, the deadline in `manager_verify.py` has expired on a board, and on
2026-09-21T23:42:11Z one of them **decided** — `verdict: confirmed`. Between the
second and the third lie four hours and thirty-nine minutes of `203/EXEC`, which
is what a defect looks like from the board's side.

What that still does not cover is the branch that matters most: **no deadline
has gone unjudged and reverted a package on hardware.** Every observed verdict
so far confirmed an install. The way back has been armed, and fired, and agreed
...
| The armed deadline decides on a board | **PASS** | it did, on the third attempt and after the defect above was fixed. read on 2026-09-22 from the board's own journal and status API over the LAN (`/api/logs/*`, `/api/status`, `/api/manager`), captured while the appliance was running: `ems-appliance-manager-verify.service` failed **255 times** with `203/EXEC` between 14:54:30Z and 19:33:41Z — four hours and thirty-nine minutes of retries at roughly seventy seconds apart, the storm the row below this one used to describe. The last failure is 19:33:41Z; the install of **0.3.2**, which carries the execute-bit fix, completes at 19:34:41Z and the unit has run cleanly since. From 23:04:22Z the reverter speaks for itself — `sh[242813]: verify-manager: not healthy yet, 899s left` — which is the snapshot executing through `/bin/sh` as the fixed unit invokes it. The **0.3.4** install armed at 23:41:10Z and the deadline settled at **23:42:11Z** with `verdict: confirmed`, `detail: "ems-appliance-manager 0.3.4 is installed and its services are running"`, `settled: true`, and `verify.armed: false`. A board has now been observed completing a verdict. What is still unproven is the other branch: **no deadline has expired unjudged and reverted a package on hardware** |
| A manager upgrade chain completes in place on a board | **PASS** | five upgrades, no reinstall and no reflash, read out of the board's own `dpkg.log`: `0.2.2 → 0.2.3` (2026-08-30 22:31:24), `0.2.3 → 0.3.1` (2026-09-20 18:34:43), `0.3.1 → 0.3.2` (2026-09-21 19:33:50), `0.3.2 → 0.3.3` (23:04:24) and `0.3.3 → 0.3.4` (23:41:06). Each reached `status installed`. Two of them crossed a minor version, and one of them carried the fix for the defect the row above describes |
...
| The built image boots on a Pi 3B+ | **PASS** | first boot 2026-08-29T22:48:35Z, and the board was still up on 2026-09-22 with an uptime of 107993 s (1 day 5 h) after an intervening reboot. read on 2026-09-22 from the board's own journal and status API over the LAN (`/api/logs/*`, `/api/status`, `/api/manager`), captured while the appliance was running. That the card came from *this* image rather than a hand-built Raspberry Pi OS is established by the grow-root row below: the partition it grew from is 8589934592 bytes, which is `root_part_size: 8G` in `packaging/appliance/image/shared/ems-appliance.yaml` exactly. Hardware as reported by the board: `Raspberry Pi 3 Model B Plus Rev 1.4`, `aarch64`, Debian 13 (trixie), kernel 6.18.39+rpt-rpi-v8, 53.7 °C, `under_voltage: false` |
| A Pi 3B+ runs Docker, Admin, EMS and InfluxDB in 1 GB of RAM | NOT RUN | **the full claim is still unmeasured: EMS and InfluxDB are not installed on the live board** (`install-state: none`), so nothing about them has been observed. What *has* been measured there, with Docker, the Admin container and the appliance services running and nothing else: 905 MB total, 248 MB used, 656 MB available; root filesystem 14816 MB, 25.2 % used. read on 2026-09-22 from the board's own journal and status API over the LAN (`/api/logs/*`, `/api/status`, `/api/manager`), captured while the appliance was running |
...
| The root is genuinely writable when `ems-appliance-agent` starts | **PASS** | the agent has been writing to it for weeks on the live board: the operations journal carries records back to 2026-04, and tonight it wrote a new one (`ssh.key_add`, operation `d9f532857334457f98c5df31d9baeb0b`) — which failed, but on `ProtectHome`, not on a read-only root. read on 2026-09-22 from the board's own journal and status API over the LAN (`/api/logs/*`, `/api/status`, `/api/manager`), captured while the appliance was running |
| The agent and the web service actually come up | **PASS** | both report `active`/`enabled`/`running: true` on the live board, and the web service answered every request this evidence was gathered with. read on 2026-09-22 from the board's own journal and status API over the LAN (`/api/logs/*`, `/api/status`, `/api/manager`), captured while the appliance was running |
