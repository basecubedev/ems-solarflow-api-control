...
This is the technical troubleshooting reference for advanced users and support.
It preserves the command-level diagnostics, log-event details and failure
analysis that are intentionally kept out of the short user guide.

For normal user workflows, see [Troubleshooting](../user/troubleshooting.md).

Related references: [control-logic.md](control-logic.md),
[control-flow.md](control-flow.md), [configuration.md](configuration.md),
[runtime-state.md](runtime-state.md), [safety-model.md](safety-model.md),
[cli.md](../cli.md), [influxdb.md](influxdb.md).
...
Full `emsctl.py` reference: [cli.md](../cli.md).
...
[install-docker.md](../install-docker.md). Open a new login session after
...
More detail: [runtime-state.md](runtime-state.md),
...
[home-assistant.md](../home-assistant.md), and
...
See [backup-restore.md](backup-restore.md) for the full workflow. Common issues:
...
- **"failed to start bundled InfluxDB via docker compose"** — Docker/Compose is
  required. Check `docker compose ps`.
...
The Admin Console guided upgrade is a conservative, single-shot workflow: it
verifies the target image, optionally backs up, optionally updates config,
rewrites the EMS image reference in `docker-compose.yml`, and force-recreates the
`ems` service. It never removes containers, volumes, or data. For the version
