...
# record is written in the image build chroot, on the build host's disk, before
# rpi-image-gen packs the tree into ext4 and reassigns inodes. Neither value
# survives that, so comparing them refused every appliance ever flashed and
# blocked every package operation on it.
home_identity() {
    [ -n "${1:-}" ] || return 0
    stat -c '%d:%i' "$1" 2>/dev/null || true
}

# Why real_chain and require_real_chain: one rule, one message, two callers --
# a predicate for the ownership question and a refusal for the destructive
# paths. Spelling the walk twice is how the two came to disagree elsewhere.
real_chain_problem=""
...
real_chain() {
    ...
    while [ -n "$rest" ]; do
        ...
        if [ -L "$prefix" ]; then
            real_chain_problem="$1 is reached through a symbolic link at $prefix"
            return 1
        fi
        [ -e "$prefix" ] || return 0
        if [ ! -d "$prefix" ]; then
            real_chain_problem="$1 passes through $prefix, which is not a directory"
            return 1
        fi
    done
    return 0
...
# The recorded home must still be a real directory at that path, reached
# without a symbolic link, carrying the marker this record names. A home that is
# gone, replaced by a file, or reached through a redirected ancestor is somebody
# else's state, whatever the passwd entry still says.
home_is_recorded() {
    ...
    # Present but never compared: see home_identity. A record missing the pair
    # was not written by this package's writer. The chain walk moved here from
    # the destructive paths because the pair was quietly carrying it -- a home
    # reached through a redirected ancestor used to surface as a pair mismatch,
    # and nothing else was looking.
...
require_real_chain() {
    real_chain "$1" || fail "$real_chain_problem"
}
