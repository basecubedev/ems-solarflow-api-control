...
def verify_zendure_endpoint(ip, port=80, timeout_ms=TIMEOUT_MS_DEFAULT, session=None,
    ...
    Returns a ``DiscoveredDevice`` on success or ``None`` on any failure, so an
    mDNS candidate is only promoted to a real device once its API confirms it.
    """
    ...
    try:
        return probe_zendure(
            session, ip, timeout_s, port=port, failure_details=failure_details
        )
